  ...
  return (
    <div className="card-elegant rounded-xl p-5">
      <div className="flex items-center justify-between">
        ...
        {icon && <div className="size-8 rounded-lg bg-surface-2 grid place-items-center text-muted-foreground">{icon}</div>}
      </div>
      <div className="mt-3 text-2xl font-semibold tracking-tight">{value}</div>
      {delta && (
        <div className={`mt-1 text-xs font-medium ${positive ? "text-success" : "text-destructive"}`}>{delta} vs last period</div>
      )}
    </div>
  );
}
...
export function EmptyState({ icon, title, description, action }: { icon?: ReactNode; title: string; description?: string; action?: ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center text-center rounded-2xl border border-dashed border-border/70 p-12 bg-surface-1/30">
      {icon && <div className="mb-3 size-12 rounded-xl bg-surface-2 grid place-items-center text-muted-foreground">{icon}</div>}
