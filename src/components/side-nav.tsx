...
export function SideNav({ items, title }: { items: SideNavItem[]; title?: string }) {
  ...
  return (
    <aside className="hidden md:block w-60 shrink-0 border-r border-border/60 h-[calc(100vh-4rem)] sticky top-16 overflow-y-auto py-6 px-3">
      {title && (
        <div className="px-2 mb-4 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
          {title}
        </div>
      )}
      <nav className="space-y-6">
        {groups.map((g) => (
          <div key={g}>
            {g && (
              <div className="px-2 mb-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/70">
                {g}
              </div>
            )}
              ...
              {items
                ...
                .map((it) => {
                  ...
                  return (
                    ...
                    >
                      <span className={cn("size-4 shrink-0", active && "text-primary")}>
                        {it.icon}
                      </span>
