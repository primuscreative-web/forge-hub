...
export const Route = createFileRoute("/marketplace")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse 12,480+ developer products across 70+ categories. Filter by price, tech stack, license, and more." },
      { property: "og:title", content: "Marketplace — DevForge Hub" },
      { property: "og:description", content: "Browse 12,480+ developer products across 70+ categories." },
    ],
  }),
...
function MarketplacePage() {
  ...
  const [filters, setFilters] = useState({ free: false, paid: false, os: false, premium: false, enterprise: false });
  ...
  const toggleCat = (slug: string) => setSelected((s) => s.includes(slug) ? s.filter((c) => c !== slug) : [...s, slug]);
        ...
        <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
          ...
          <aside className="space-y-6">
            <div className="card-elegant rounded-xl p-5 space-y-5">
              <div className="flex items-center gap-2 text-sm font-medium"><SlidersHorizontal className="size-4" />Filters</div>
              ...
              <FilterGroup title="Product type">
                {[
                  ...
                  <label key={f.k} className="flex items-center gap-2 text-sm cursor-pointer">
                    <Checkbox checked={filters[f.k as keyof typeof filters]} onCheckedChange={(v) => setFilters({ ...filters, [f.k]: !!v })} />
                    {f.label}
                  </label>
                ))}
              </FilterGroup>
                  ...
                  <label key={r} className="flex items-center gap-2 text-sm cursor-pointer">
                    ...
                    <div className="flex items-center gap-0.5">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={`size-3 ${i < Math.floor(r) ? "fill-warning text-warning" : "text-muted-foreground/40"}`} />
                      ))}
                    </div>
                ...
                <div className="max-h-64 overflow-y-auto pr-2 space-y-1.5">
                  {categories.slice(0, 30).map((c) => (
                    <label key={c.slug} className="flex items-center gap-2 text-sm cursor-pointer">
                      <Checkbox checked={selected.includes(c.slug)} onCheckedChange={() => toggleCat(c.slug)} />
          ...
          <div className="min-w-0 space-y-5">
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative flex-1 min-w-[240px]">
                ...
                <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search products, tags, tech…" className="pl-9 h-10 bg-surface-1" />
              </div>
              ...
              <Tabs value={view} onValueChange={(v) => setView(v as "grid" | "list")}>
                <TabsList>
                  ...
                  <TabsTrigger value="list"><List className="size-4" /></TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
            ...
            {selected.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                {selected.map((s) => (
                  <Badge key={s} variant="outline" className="gap-1 cursor-pointer" onClick={() => toggleCat(s)}>
                    {categories.find((c) => c.slug === s)?.name}
                    <span className="text-xs">×</span>
                  </Badge>
                ))}
                <Button variant="ghost" size="sm" onClick={() => setSelected([])} className="h-6 text-xs">Clear all</Button>
              </div>
            )}
            ...
            {filtered.length === 0 ? (
              ...
              <div className="space-y-2">
                {filtered.map((p) => (
                  <Link key={p.id} to="/product/$slug" params={{ slug: p.slug }} className="flex items-center gap-4 card-elegant rounded-xl p-4 hover-lift hover:[&]:hover-lift-active">
                    <div className="size-16 rounded-lg grid place-items-center text-3xl" style={{ background: p.gradient }}>{p.emoji}</div>
                    <div className="flex-1 min-w-0">
                      <div className="font-medium">{p.name}</div>
                      <div className="text-xs text-muted-foreground truncate">{p.tagline}</div>
                    </div>
                    <div className="hidden md:flex items-center gap-1 text-xs text-muted-foreground">
                      <Star className="size-3 fill-warning text-warning" />{p.rating} · {p.sales} sales
                    </div>
