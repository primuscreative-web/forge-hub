...
export const Route = createFileRoute("/admin/orders")({
  ...
  component: () => {
    const rows = Array.from({ length: 12 }).map((_, i) => ({
      id: `ORD-${1000 + i}`,
      buyer: ["Elena Costa", "Kaito Rivera", "Prism Agency", "CloudForge"][i % 4],
      init: ["EC", "KR", "PA", "CF"][i % 4],
      product: ["Nexus Dashboard Pro", "AuthForge", "Prism UI Kit", "Cortex AI Agent Kit"][i % 4],
      amount: [149, 249, 89, 399][i % 4],
      date: `2026-07-${(i % 28) + 1}`,
      status: (["Paid", "Paid", "Refunded", "Paid"] as const)[i % 4],
    }));
    return (
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                key: "b",
                header: "Buyer",
                render: (r) => <UserCell name={r.buyer} initials={r.init} />,
              },
              {
                key: "p",
                header: "Product",
              ...
              },
              {
                key: "a",
                header: "Amount",
                render: (r) => <span className="text-sm font-medium">${r.amount}</span>,
              },
              {
                key: "d",
                header: "Date",
