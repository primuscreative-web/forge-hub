...
export const Route = createFileRoute("/dashboard/collections")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Curate collections of your favorite products, publicly or privately.",
      },
    ...
    const collections = [
      {
        ...
        desc: "Everything for launching a SaaS in a weekend.",
        items: 6,
        cover: [0, 4, 3],
      },
      {
        name: "AI toolkit",
        desc: "MCP servers, prompts, agent frameworks.",
        items: 8,
        cover: [2, 7, 15],
      },
      {
        name: "Design essentials",
        desc: "UI kits, icons, Figma resources.",
        items: 5,
        cover: [3, 8, 19],
      },
      {
        name: "Cloud infra",
        desc: "Terraform, K8s, Cloudflare setups.",
        items: 4,
        cover: [9, 27, 5],
      },
    ];
    return (
      <DashboardLayout side={<SideNav items={buyerNav} title="Personal" />}>
        <div className="p-6 md:p-8 max-w-[1200px]">
          <PageHeader
            title="Collections"
            description="Organize your favorite products into curated sets."
            actions={
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {collections.map((c) => (
              <Link
                key={c.name}
                to="/collections"
                className="card-elegant rounded-2xl overflow-hidden hover-lift hover:[&]:hover-lift-active"
              >
