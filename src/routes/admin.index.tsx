...
import { DollarSign, Users, ShoppingBag, Package, ShieldAlert, MessageSquare, Activity } from "lucide-react";
    ...
    <DashboardLayout side={<SideNav items={adminNav} title="Admin" />}>
      <div className="p-6 md:p-8 max-w-[1400px]">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
          <StatCard label="MRR" value="$1.24M" delta="+8.2%" icon={<DollarSign className="size-4" />} />
          <StatCard label="Active users" value="248,400" delta="+12.1%" icon={<Users className="size-4" />} />
          <StatCard label="Orders (30d)" value="42,180" delta="+9.4%" icon={<ShoppingBag className="size-4" />} />
          <StatCard label="Products" value="12,480" delta="+184 new" icon={<Package className="size-4" />} />
        </div>
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="card-elegant rounded-2xl p-6 space-y-3">
            ...
            <QuickLink to="/admin/moderation" icon={<ShieldAlert className="size-4" />} label="Moderation queue" value="12" />
            <QuickLink to="/admin/tickets" icon={<MessageSquare className="size-4" />} label="Support tickets" value="24 open" />
            <QuickLink to="/admin/reports" icon={<Activity className="size-4" />} label="User reports" value="8" />
          </div>
        </div>
      </div>
    </DashboardLayout>
...
function QuickLink({ to, icon, label, value }: { to: string; icon: React.ReactNode; label: string; value: string }) {
  return (
    <Link to={to} className="flex items-center gap-3 p-3 rounded-lg hover:bg-accent/40 transition-colors">
      <div className="size-8 rounded-lg bg-surface-2 grid place-items-center text-muted-foreground">{icon}</div>
