  ...
  component: () => (
    <DashboardLayout side={<SideNav items={adminNav} title="Admin" />}>
      <div className="p-6 md:p-8 max-w-[1400px]">
        <PageHeader title="Creators" description={`${creators.length} creators · ${creators.filter(c => c.verified).length} verified`} />
        <DataTable rows={creators.map(c => ({ ...c }))} columns={[
          { key: "u", header: "Creator", render: (r) => <UserCell name={r.name} sub={"@" + r.handle} initials={r.avatar} /> },
