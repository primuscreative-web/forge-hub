...
export const Route = createFileRoute("/creator/analytics")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Deep analytics for your products: views, downloads, conversions, revenue, and customer segments." },
  ...
  return (
    <DashboardLayout side={<SideNav items={creatorNav} title="Creator" />}>
      <div className="p-6 md:p-8 max-w-[1400px]">
        <PageHeader title="Analytics" description="Insights across all your products, updated in real-time." />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
          <StatCard label="Total views" value="248,400" delta="+14.2%" icon={<Eye className="size-4" />} />
          <StatCard label="Unique visitors" value="82,140" delta="+8.6%" icon={<Users className="size-4" />} />
          <StatCard label="Conversion rate" value="4.8%" delta="+0.4pp" icon={<TrendingUp className="size-4" />} />
          <StatCard label="Avg. order value" value="$147" delta="+$12" icon={<BarChart3 className="size-4" />} />
        </div>
