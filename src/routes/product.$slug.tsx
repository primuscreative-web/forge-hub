...
function ProductPage() {
  ...
  const live = product as typeof product & { thumbnailUrl?: string; demoUrl?: string; repositoryUrl?: string; documentationUrl?: string; productType?: string; gallery?: string[] };
  const freeAccess = product.free;
  return <SiteLayout><div className="mx-auto max-w-6xl px-4 md:px-6 py-10">
    <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
      <main><div className="aspect-[16/8] overflow-hidden rounded-3xl border bg-muted">{live.thumbnailUrl ? <img src={live.thumbnailUrl} alt="" className="h-full w-full object-cover" /> : <div className="h-full grid place-items-center text-7xl" style={{ background: product.gradient }}>{product.emoji}</div>}</div>
        ...
        {(live.gallery?.length ?? 0) > 0 && <section className="mt-6 grid gap-4 sm:grid-cols-2">{live.gallery!.map((url) => <img key={url} src={url} alt="" className="w-full rounded-2xl border object-cover" />)}</section>}
