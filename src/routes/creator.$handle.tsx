...
export const Route = createFileRoute("/creator/$handle")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!c)
      return {
        meta: [{ title: "Creator · DevForge Hub" }, { name: "robots", content: "noindex" }],
      };
    return {
      meta: [
      ...
      <div className="mx-auto max-w-[1440px] px-4 md:px-6 -mt-16 md:-mt-20">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="flex items-end gap-4">
            <div className="size-24 md:size-32 rounded-3xl gradient-brand grid place-items-center text-white text-3xl font-semibold ring-4 ring-background">
              {creator.avatar}
            </div>
            <div className="pb-2">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[320px_1fr]">
          <aside className="space-y-4">
            <div className="card-elegant rounded-2xl p-5 space-y-4">
              ...
              <div className="space-y-2 text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  ...
                  Joined{" "}
                  {new Date(creator.joined).toLocaleDateString("en", {
                    month: "long",
                    year: "numeric",
                  })}
                </div>
                {creator.organization && (
                  <div className="flex items-center gap-2">
                    <Badge variant="outline">{creator.organization}</Badge>
                  </div>
                )}
              </div>
            </div>
            <div className="card-elegant rounded-2xl p-5 grid grid-cols-2 gap-3 text-center">
              ...
              <Stat
                label="Rating"
                value={String(creator.rating)}
                icon={<Star className="size-3 fill-warning text-warning inline" />}
              />
            </div>
            ...
            <Tabs defaultValue="products">
              ...
              <TabsContent value="products" className="mt-6">
                {creatorProducts.length ? (
                  ...
                  <div className="card-elegant rounded-xl p-10 text-center text-sm text-muted-foreground">
                    No products yet.
                  </div>
                )}
              </TabsContent>
              ...
              <TabsContent value="reviews" className="mt-6">
                <div className="card-elegant rounded-xl p-10 text-center text-sm text-muted-foreground">
                  4.9 average across 3,240 reviews.
                </div>
              </TabsContent>
              <TabsContent value="activity" className="mt-6">
                <div className="space-y-2 text-sm">
                  {[
                    "Published Nexus Dashboard Pro v2.4.0",
                    "Released Cortex AI Agent Kit",
                    "Reached 24,800 followers",
                    "Received 4.9★ average rating",
                  ].map((a, i) => (
