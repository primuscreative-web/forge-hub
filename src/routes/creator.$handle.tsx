...
function CreatorProfile() {
  ...
  return <SiteLayout><div className="h-40 md:h-56 gradient-hero border-b bg-cover bg-center" style={creator.coverUrl ? { backgroundImage: `url(${creator.coverUrl})` } : undefined} /><div className="mx-auto max-w-[1440px] px-4 md:px-6 -mt-16 md:-mt-20 pb-16">
    <div className="flex items-end gap-4"><div className="size-24 md:size-32 rounded-3xl gradient-brand grid place-items-center overflow-hidden text-white text-3xl font-semibold ring-4 ring-background">{creator.avatarUrl ? <img src={creator.avatarUrl} alt={creator.name} className="h-full w-full object-cover" /> : creator.avatar}</div><div className="pb-2"><h1 className="text-2xl md:text-3xl font-semibold">{creator.name}</h1><p className="text-sm text-muted-foreground">@{creator.handle}</p></div></div>
