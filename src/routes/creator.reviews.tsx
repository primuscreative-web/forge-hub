...
export const Route = createFileRoute("/creator/reviews")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Read and respond to reviews across your published products." },
...
function Reviews() {
  const all = [...reviews, ...reviews].map((r, i) => ({ ...r, product: products[i % products.length].name }));
  return (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">on {r.product} · {r.date}</span>
                  </div>
                  <div className="flex items-center gap-0.5 mt-0.5">
                    {Array.from({ length: 5 }).map((_, j) => <Star key={j} className={`size-3 ${j < r.rating ? "fill-warning text-warning" : "text-muted-foreground/40"}`} />)}
                  </div>
