...
export const TicketsRoute = createFileRoute("/admin/tickets")({
  ...
  component: () => {
    const rows = Array.from({ length: 10 }).map((_, i) => ({
      id: `TKT-${2400 + i}`, subject: ["License issue","Refund request","Bug report","Feature question","Payout question"][i%5],
      user: ["Elena Costa","Kaito Rivera","Prism Agency","CloudForge"][i%4],
      init: ["EC","KR","PA","CF"][i%4], priority: (["High","Medium","Low"] as const)[i%3],
      status: (["Open","Under review","Resolved"] as const)[i%3], updated: `${i+1}h ago`,
    }));
    return (
      <DashboardLayout side={<SideNav items={adminNav} title="Admin" />}>
        <div className="p-6 md:p-8 max-w-[1400px]">
          ...
          <DataTable rows={rows} columns={[
            ...
            { key: "s", header: "Subject", render: (r) => <span className="text-sm font-medium">{r.subject}</span> },
            { key: "u", header: "User", render: (r) => <UserCell name={r.user} initials={r.init} /> },
            { key: "p", header: "Priority", render: (r) => <Badge variant="outline">{r.priority}</Badge> },
            { key: "st", header: "Status", render: (r) => <StatusBadge status={r.status} /> },
            { key: "up", header: "Updated", render: (r) => <span className="text-xs text-muted-foreground">{r.updated}</span> },
          ]} />
        </div>
      </DashboardLayout>
    );
